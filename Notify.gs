...
function enviarRelatorioDiarioGatilhos() {
  ...
  for (const email in setoresPorEmail) {
    ...
    try {
      MailApp.sendEmail(Object.assign({ to: email }, _camposRemetenteEmail_(), { subject: assunto, htmlBody: html }));
    } catch (e) {
...
function _enviarNovaDemandaEspontanea_(caso) {
  ...
  MailApp.sendEmail(Object.assign({ to: emailsDestino.join(',') }, _camposRemetenteEmail_(), { subject: assunto, htmlBody: html }));
}
...
function _enviarInvestigacaoConcluida_(caso) {
  ...
  MailApp.sendEmail(Object.assign({ to: emailNotificador }, _camposRemetenteEmail_(), { subject: assunto, htmlBody: html }));
}
...
function enviarEmailTeste(tipo, destinatario, token) {
  return _comAdmin_(token, function () {
    ...
    MailApp.sendEmail(Object.assign({ to: email }, _camposRemetenteEmail_(), { subject: '[TESTE] ' + montado.assunto, htmlBody: montado.html }));
